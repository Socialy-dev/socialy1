...
import { Briefcase, Calendar, Building2, Euro, ExternalLink, Clock, FileText, Loader2, RefreshCw, Search, TrendingUp, Check, Star, X, Users } from "lucide-react";
...
export const MarchePublicView = () => {
  ...
  const fetchLastVisit = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("user_marche_last_visit" as any).select("last_visit_at").eq("user_id", user.id).eq("organization_id", effectiveOrgId).maybeSingle();
  ...
  const updateLastVisit = async () => {
    ...
    try {
      const {
        error
      } = await supabase.from("user_marche_last_visit" as any).upsert({
        user_id: user.id,
  ...
  const fetchMarches = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("organization_marche_public").select("*").eq("organization_id", effectiveOrgId).order("date_publication", {
        ascending: false
      });
      if (error) throw error;
      setMarches(data || []);
  ...
  const fetchSelections = async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.from("user_marche_selections" as any).select("id, user_id, marche_public_id, status").eq("organization_id", effectiveOrgId);
      if (error) throw error;
      ...
      if (otherSelectionsLocal.length > 0) {
        const {
          data: teamData,
          error: teamError
        } = await supabase.functions.invoke("get-team-marche-selections", {
          body: {
            organization_id: effectiveOrgId
          }
        });
    ...
    try {
      if (existingSelection) {
        if (existingSelection.status === "selected") {
          const {
            error
          } = await supabase.from("user_marche_selections" as any).delete().eq("id", existingSelection.id);
          if (error) throw error;
          ...
          const {
            error
          } = await supabase.from("user_marche_selections" as any).update({
            status: "selected"
          } as any).eq("id", existingSelection.id);
          if (error) throw error;
          setSelections(prev => prev.map(s => s.id === existingSelection.id ? {
            ...s,
            status: "selected"
          } : s));
        ...
        const {
          data,
          error
        } = await supabase.from("user_marche_selections" as any).insert({
          user_id: user.id,
          organization_id: effectiveOrgId,
  ...
  const filteredMarches = marches.filter(m => {
    const matchesSearch = !searchQuery || m.titre?.toLowerCase().includes(searchQuery.toLowerCase()) || m.acheteur?.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  return <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className={cn("rounded-2xl p-5 border cursor-pointer transition-all duration-300", viewMode === "nouveaux" ? "bg-gradient-to-br from-emerald-500/20 to-emerald-500/10 border-emerald-500/40 ring-2 ring-emerald-500/30" : "bg-gradient-to-br from-emerald-500/10 to-emerald-500/5 border-emerald-500/20 hover:border-emerald-500/40")} onClick={() => setViewMode(viewMode === "nouveaux" ? "all" : "nouveaux")}>
          ...
          <div className="flex items-center justify-between">
            ...
            {newMarchesCount > 0 && <Badge className="bg-emerald-500 text-white border-0">
                {viewMode === "nouveaux" ? "Voir tout" : "Voir"}
              </Badge>}
          </div>
        </div>

        <div className={cn("rounded-2xl p-5 border cursor-pointer transition-all duration-300", viewMode === "selected" ? "bg-gradient-to-br from-amber-500/20 to-amber-500/10 border-amber-500/40 ring-2 ring-amber-500/30" : "bg-gradient-to-br from-amber-500/10 to-amber-500/5 border-amber-500/20 hover:border-amber-500/40")} onClick={() => setViewMode(viewMode === "selected" ? "all" : "selected")}>
          <div className="flex items-center gap-3 mb-3">
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <Input placeholder="Rechercher un marché..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 bg-card border-border" />
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {sources.map(source => <Button key={source} variant={sourceFilter === source ? "default" : "outline"} size="sm" onClick={() => setSourceFilter(sourceFilter === source ? null : source)} className="rounded-full">
              {source}
            </Button>)}
      ...
      {filteredMarches.length === 0 ? <div className="text-center py-16 bg-secondary/20 rounded-2xl">
          <Briefcase className="w-12 h-12 text-muted-foreground/30 mx-auto mb-4" />
          <p className="text-muted-foreground font-medium">
            {viewMode === "nouveaux" ? "Aucun nouveau marché" : viewMode === "selected" ? "Aucun marché en favoris" : viewMode === "team" ? "Aucun favori d'équipe" : viewMode === "dismissed" ? "Aucun marché masqué" : "Aucun marché public trouvé"}
          </p>
          <p className="text-sm text-muted-foreground/70 mt-1">
            {viewMode === "nouveaux" ? "Tous les marchés ont été consultés lors de votre dernière visite" : viewMode === "selected" ? "Cliquez sur l'étoile pour ajouter des marchés en favoris" : viewMode === "team" ? "Les marchés sélectionnés par votre équipe apparaîtront ici" : viewMode === "dismissed" ? "Les marchés masqués apparaîtront ici" : "Les opportunités apparaîtront ici automatiquement"}
          </p>
        ...
        return <div key={marche.id} className={cn("group relative bg-card rounded-3xl border p-6 transition-all duration-300 hover:shadow-xl hover:shadow-primary/5", isSelected ? "border-amber-500/50 ring-2 ring-amber-500/20" : teamMembers.length > 0 ? "border-violet-500/30" : "border-border hover:border-primary/30", isDeadlinePassed(marche.deadline) && "opacity-60")}>
                <div className="absolute top-4 right-4 flex gap-2">
                  <button onClick={e => {
              e.stopPropagation();
              toggleSelection(marche.id);
            }} className={cn("w-8 h-8 rounded-full flex items-center justify-center transition-all duration-200", isSelected ? "bg-amber-500 text-white shadow-lg shadow-amber-500/30" : "bg-muted/50 text-muted-foreground hover:bg-amber-500/20 hover:text-amber-600")}>
                    <Star className={cn("w-4 h-4", isSelected && "fill-current")} />
                  </button>
                  {status !== "dismissed" && <button onClick={e => {
              e.stopPropagation();
              dismissMarche(marche.id);
                ...
                <div className="flex items-start gap-3 mb-4 pr-20">
                  {isSelected && <div className="w-6 h-6 rounded-full bg-amber-500/20 flex items-center justify-center shrink-0 mt-0.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                      ...
                      {marche.source && <Badge variant="outline" className={cn("text-xs font-medium", getSourceColor(marche.source))}>
                          {marche.source}
                        </Badge>}
                      {marche.external_id && <span className="text-xs text-muted-foreground">Réf: {marche.external_id}</span>}
                      {isDeadlineClose(marche.deadline) && !isDeadlinePassed(marche.deadline) && <Badge className="bg-orange-500/10 text-orange-600 border-orange-500/20 text-xs">
                          <Clock className="w-3 h-3 mr-1" />
                          Urgent
                        </Badge>}
                      {isDeadlinePassed(marche.deadline) && <Badge className="bg-red-500/10 text-red-600 border-red-500/20 text-xs">
                          Expiré
                        </Badge>}
                    </div>
                  </div>
                ...
                {teamMembers.length > 0 && <div className="mb-4 p-3 bg-violet-500/5 rounded-xl border border-violet-500/10">
                    <div className="flex items-center gap-2 mb-2">
                      <Users className="w-4 h-4 text-violet-600" />
                      <span className="text-xs font-medium text-violet-600">Sélectionné par l'équipe</span>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {teamMembers.map(member => <div key={member.id} className="flex items-center gap-1.5 bg-violet-500/10 rounded-full px-2 py-1">
                ...
                <div className="space-y-3">
                  {marche.acheteur && <div className="flex items-start gap-2">
                      <Building2 className="w-4 h-4 text-muted-foreground mt-0.5 shrink-0" />
                      <span className="text-sm text-foreground line-clamp-2">{marche.acheteur}</span>
                    </div>}

                  <div className="flex items-center gap-4 flex-wrap">
                    {marche.montant !== null && <div className="flex items-center gap-2">
                        <Euro className="w-4 h-4 text-emerald-500" />
                        <span className="text-sm font-semibold text-emerald-600">{formatMontant(marche.montant)}</span>
                      </div>}

                    {marche.date_publication && <div className="flex items-center gap-2">
                        <Calendar className="w-4 h-4 text-muted-foreground" />
                        <span className="text-sm text-muted-foreground">{formatDate(marche.date_publication)}</span>
                      </div>}

                    {marche.deadline && <div className={cn("flex items-center gap-2", isDeadlineClose(marche.deadline) && !isDeadlinePassed(marche.deadline) && "text-orange-600")}>
                ...
                {marche.url && <div className="mt-4 pt-4 border-t border-border">
                    <a href={marche.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-sm text-primary hover:text-primary/80 font-medium transition-colors">
