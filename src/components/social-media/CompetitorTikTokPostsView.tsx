...
interface CompetitorTikTokPostsViewProps {
  selectedCompetitorId?: string;
}

export const CompetitorTikTokPostsView = ({ selectedCompetitorId }: CompetitorTikTokPostsViewProps) => {
  ...
  if (posts.length === 0) {
    return (
      <div className="text-center py-16 rounded-3xl bg-card border border-dashed border-border">
        ...
        <h3 className="text-lg font-semibold text-foreground mb-2">Aucune vidéo TikTok concurrent</h3>
        <p className="text-sm text-muted-foreground max-w-sm mx-auto">
          Les vidéos TikTok de vos concurrents apparaîtront ici une fois synchronisées
        </p>
      </div>
    );
  }
                ...
                <div className="px-4 pb-4 space-y-3 border-t border-border/50">
                  {post.music_name && (
                    <div className="flex items-center gap-2 text-sm text-muted-foreground pt-3">
                      ...
                      <span className="truncate">{post.music_name}</span>
                    </div>
                  )}
