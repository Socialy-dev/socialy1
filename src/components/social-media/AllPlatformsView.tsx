...
export const AllPlatformsView = () => {
    ...
    const fetchAllPosts = async () => {
      ...
      const [tiktokRes, facebookRes, linkedinRes, instagramRes, allTiktokRes, allInstagramRes] = await Promise.all([
        ...
        supabase
          ...
          .select("id, post_id, post_url, caption, likes_count, comments_count, views_count, video_play_count, images, video_url, profile_picture_url, company_name, content_type, posted_at")
          .eq("organization_id", effectiveOrgId)
          .order("posted_at", { ascending: false })
          .limit(20),
        supabase
  ...
  const getInstagramMedia = (post: InstagramPost): { url: string | null; isVideo: boolean } => {
    if (post.images) {
      ...
      if (imagesArray.length > 0) {
        return { url: imagesArray[0], isVideo: false };
      }
    }
    if (post.video_url) {
      return { url: post.video_url, isVideo: true };
    }
    if (post.profile_picture_url) return { url: post.profile_picture_url, isVideo: false };
        ...
        <div className="space-y-4">
          ...
          >
            {instagramPosts.map((post, index) => {
              const media = getInstagramMedia(post);
              const isVideoPost = post.content_type?.toLowerCase() === 'video' || post.content_type === 'Reel';
              return (
                <a
                  key={post.id}
                  href={post.post_url}
