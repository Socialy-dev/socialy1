...
export const useCompetitorInstagramAnalytics = (selectedCompetitorId?: string) => {
  ...
  const hashtagPerformance: HashtagPerformance[] = useMemo(() => {
    const hashtagMap = new Map<string, { posts: CompetitorInstagramPost[] }>();
  ...
  const contentTypeStats: ContentTypeStats[] = useMemo(() => {
    const typeMap = new Map<string, CompetitorInstagramPost[]>();
